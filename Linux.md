...
* 任务调度和分类：1.系统工作：有些重要的工作必须周而复始地执行。如病毒扫描等。2.个别用户工作：个别用户可能希望执行某些程序，比如mysql数据库地备份。

只是简单的任务，可以不用写脚本，只截在crontab中加入任务即可。对于复杂任务需要写shell脚本。

crond -e  加执行任务  -l 列出任务 -r 删除任务

##### Linux磁盘分区和挂载

！！！

查询系统整体磁盘使用情况  df -h   du
